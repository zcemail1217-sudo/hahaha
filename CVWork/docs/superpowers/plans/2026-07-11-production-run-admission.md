...
除上面的 public seam/source guard 外，再增加一个真实生命周期行为测试：

- 用 `FakeInspectionExecution.PublishCompleted` 从后台线程发布包含运行值 A 的结果，断言 `VariableCenterViewModel.RuntimeValues` 只在 `IUiDispatcher.Invoke` 边界内更新。
- 调用 `VariableCenterViewModel.Dispose()` 后发布运行值 B，断言结果仍保留 A，证明 `RunCompleted` 已退订。
- 给 `RecordingCommunicationChannels` 增加可配置 `ConnectHandler`。测试让 live polling 停在接受 ViewModel cancellation token 的 `ConnectAsync` 中；Dispose 后等待取消完成信号，保证 fire-and-forget polling 确定性退出，不遗留后台任务或未观察异常。
...
5. 把 `VisionStation.Application.Tests/InspectionRunnerRecipeResolutionTests.cs` 中对真实 Runner 的全部 7 处 `RunAsync` 调用同步改为 `ExecuteAsync`，并把全部 7 个 `RunAsync_*` 测试名改成 `ExecuteAsync_*`；该测试程序集已有 `InternalsVisibleTo`，不需要反射或兼容 adapter。
...
```powershell
rg -n -g "*.cs" "\bIInspectionRunner\b|_inspectionRunner|\.RunAsync\s*\(" .\VisionStation.Application .\VisionStation.Application.Tests .\VisionStation.Client .\VisionStation.Vision.UI .\VisionStation.Vision.UI.Tests
rg -n -g "*.cs" "\bRunAsync_" .\VisionStation.Application.Tests\InspectionRunnerRecipeResolutionTests.cs
rg -n -g "*.cs" "RunCompleted\?\.Invoke" .\VisionStation.Application
```

Expected: 三条命令均无结果；检测执行只通过 `session.ExecuteAsync`，结果事件只由 `InspectionExecution` 安全发布。
