...
确认 Task 12 composition 注册的 Service/Store/Resources 与 pipeline/dialog/recipe service 引用相同对象。`App.OnExit` 保持 Task 5 已加入的顺序：先取消视觉任务，再等待 `_templateMatchingService.DisposeAsync()`，最后释放 communication runtime；异常使用 `IAppLogService.Error(nameof(App), message)` 记录，不弹窗。
...
- Modify: `VisionStation.Vision.Halcon.TestHost/Program.cs`
...
`HalconBenchmarkReport` 与 `LocalHalconDatasetManifest` 是 TestHost 程序集中的 public 纯类型，Program 只做参数/退出码/JSON 编排，测试通过 Task 18 的 ProjectReference 直接复用。另锁定现场 manifest 的 label 只允许 positive/front、back、similar、partial、boundary、polarity，未知标签 fail closed；输出 JSON 必含机器/软件指纹和 cold/warm/1/3/5 targets 分组。
...
`LocalHalconDatasetAcceptanceTests` 只在显式 `VISIONSTATION_HALCON_DATASET` 目录存在时运行；目录内用 manifest 标注 positive/front、back、similar、partial、boundary、polarity 等预期。代码不写本机绝对路径、不把图片复制进仓库；负例首要断言零接受，正例记录漏检。
...
```powershell
rg -n "ITemplateMatchingService|CONFIG_UNKNOWN_ENGINE|MATCH_TIMEOUT|matchesV2|Pose.Scale|HALCONROOT|26050.0.0|26.05.0.0" docs\development\halcon-shape-*.md
$markers = @('T' + 'ODO', 'T' + 'BD', '待' + '补', '占' + '位') -join '|'
rg -n "D:\\\\HALCON|C:\\\\Users\\\\Time|$markers" docs\development\halcon-shape-*.md
```
...
```powershell
git status --short
git add docs/development/halcon-shape-matching.md docs/development/halcon-shape-deployment.md docs/development/halcon-shape-performance-baseline.md VisionStation.Vision.Halcon.TestHost/Program.cs VisionStation.Vision.Halcon.TestHost/HalconBenchmarkReport.cs VisionStation.Vision.Halcon.TestHost/LocalHalconDatasetManifest.cs VisionStation.Vision.Halcon.Tests/LocalHalconDatasetAcceptanceTests.cs VisionStation.Vision.Halcon.Tests/HalconBenchmarkCommandTests.cs
