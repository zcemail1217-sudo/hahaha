...
`TemplateMatchResult` 在 record body 中增加可选 init-only 诊断属性，保持现有位置构造函数与 `Deconstruct` 不变：
...
现有 `ShapeContours` 的语义收紧为“真正参与评分的模板边缘”。新增属性默认空值，不要求 v1、Managed 或 ORB 调用方提供诊断数据。
...
- 评分边缘：橙色。
- 完整模板 ROI：使用专用 `Info` 状态并显示为青色。不能复用 `Neutral`，因为生产结果预览会过滤配方编辑用的 Neutral ROI。
- 定位中心：按最终结果显示绿色或红色。
- 定位标签显示最终分数和覆盖率。

生产看板与流程调试页当前各自复制了一套“过滤结果叠加并清空标签”的逻辑。本阶段提取共享 `VisionResultOverlayProjector`：过滤方向轴和配方编辑用 Neutral ROI，保留 Info 模板 ROI，并只保留定位 Cross 的诊断标签。两个页面使用同一规则，避免后续显示语义再次分叉。
...
- `VisionStation.Vision.UI/Services/VisionResultOverlayProjector.cs`（新增）
...
## 14. 终审兼容性补充（2026-07-14）

整分支终审补充以下不可省略的兼容契约：

1. `MatchedTemplateRoiContours` 必须覆盖默认 `Rectangle`、`RotatedRectangle`、`Circle` 和 `Polygon`。矩形 ROI 使用左上角 `X/Y` 与 `Width/Height` 生成四角，和现有 ROI 坐标语义一致。
2. 缺少 `shapeScoreVersion` 或显式为 `1` 时，V1 默认 `shapeScoreScale` 继续由当前 pass 的未旋转模板短边决定。keep-bounds 只修复几何裁剪，不得让旧分数尺度随角度画布变化；V2 继续使用旋转画布尺度。
3. `MultiTargetMatch` 未接入 Shape V2。即使共享学习入口返回 `shapeScoreVersion` 和 `shapeCoverageDistance`，MultiTarget 构建运行/保存参数时也必须剔除这两个单目标字段。
4. 新 `ToolResult.Data` 写入 `hasMatch`。`hasMatch=false` 表示没有真实候选，UI 禁止创建定位 Cross 或回退矩形；已有轮廓仍可独立容错显示。
5. 旧历史没有 `hasMatch` 时，只有 `x`、`y`、`angle`、`score` 四个主字段都存在且为有限数值，才兼容推断为可显示候选。这样保留真实低分 NG 候选，同时避免缺输入或损坏记录在 `(0,0)` 伪造定位。

对应实施步骤和 Red-Green 命令见 `docs/superpowers/plans/2026-07-14-opencv-shape-v2-final-review-fixes.md`。
