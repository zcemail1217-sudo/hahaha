...
- `RecipeSnapshot is not null`：Runner 不调用 `IRecipeRepository` 的任何成员，直接规范化并执行该快照。
...
1. 如果请求带快照，先验证 ID 一致性，再返回 `RecipeSnapshot.WithNormalizedFlows()`；整个快照解析与执行路径不得调用 `IRecipeRepository` 的任何成员。
...
- attempt 返回后必须再次观察 attempt cancellation，并在取消完成后最终仲裁 Reset latch；即使执行器忽略已取消 token 并迟到返回成功，已接受的 Reset 仍优先启动默认值 attempt，导航取消仍优先结束 owner 生命周期。
...
`IsTestRunning` 必须保持为 `true`，编辑区必须保持冻结，直到 `IInspectionSession.DisposeAsync` 成功完成并确认 execution owner 已释放。若 Dispose 失败且 `IInspectionExecution.Current` 仍指向同一 Session，页面必须 fail closed：继续展示 owner、继续冻结编辑，不能伪报空闲，也不能盲目重复 Dispose；只有实现特定的恢复流程或外部确认 owner 已释放后才能解冻。

Pause/Resume 的同步边界同样分段隔离：共享 `IInspectionRunControl` 调用失败时不得伪造暂停/恢复状态；控制调用已经成功后，属性通知或日志订阅者失败不得撤销真实控制状态，也不得让异常逃逸 WPF 命令边界。
...
- Pause/Resume 同步控制报错：隔离报告，保留共享 RunControl 的真实状态，异常不得逃逸命令边界。
- `CanExecuteChanged` 订阅者报错：安全记录，不能传播进 attempt/finally 生命周期。
- EndRun、Disconnect、日志和 Session Dispose 继续保持分段隔离及既有有界清理语义；Dispose 成功前 owner 不得解冻，Dispose 失败且 `Current` 仍为同一 Session 时 fail closed。

## 10. 测试设计

### 10.1 Application

- 请求带快照时，Runner 不访问 `IRecipeRepository` 的任何成员，实际结果来自快照；测试桩对全部仓库成员统一计数。
...
- 执行器忽略已取消 token 并迟到返回成功时，已接受 Reset 仍由最终 latch 仲裁触发默认值 attempt；导航取消仍结束 owner 生命周期。
- 最终 Disconnect cleanup 期间 Reset 的 CanExecute 与方法内执行均被拒绝。
- Session Dispose 未完成时 `IsTestRunning` 和编辑冻结保持不变；Dispose 失败且 `Current` 仍为同一 Session 时保持 fail-closed owner 真相。
- Pause 同步控制、属性通知或日志订阅者抛错时，命令不向 WPF 传播异常，页面投影与共享 RunControl 的已提交真相一致。
...
- 快照入口不得调用 `IRecipeRepository` 的任何成员；新增仓库 API 时也要同步扩展全成员调用守卫。
- 不要在命令 Catch 中修改 Session owner 状态。
- 二次实现 `IInspectionSession` 时，只有 Dispose 成功完成才能表示 owner 已从 `IInspectionExecution.Current` 释放；Dispose 失败且仍为同一 owner 时，调用方必须 fail closed，不能伪报空闲或盲目重复释放。
